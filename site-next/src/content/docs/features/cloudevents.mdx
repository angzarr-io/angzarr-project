...
A CloudEvents projector filters sensitive fields and publishes public versions. Return `None` from a handler to skip publishing; return a CloudEvent to publish it.

A full Python example lives in the framework's `prj-cloudevents` crate (Rust); a Python reference implementation is forthcoming.
