...
flowchart LR
    EB[EventBook] --> P
    P --> PR[Projection<br/>wraps CloudEventsResponse<br/>in .projection]
    PR --> C
    C --> S[Sink<br/>HTTP or Kafka]

    subgraph client["Client code"]
        P["Projector<br/>@projects methods<br/>filter / transform"]
    end

    subgraph framework["Framework"]
        C["CloudEventsCoordinator<br/>unwraps Any → JSON<br/>fills envelope<br/>publishes"]
    end

    classDef plain fill:transparent,stroke:#888,color:inherit;
    class EB,PR,S plain
```

**Client projectors** use the `Projector` base class with `@projects` methods to:
...
CloudEvents projectors are built the same way as ordinary projectors: a class derived from the client library's `Projector` base, with `@projects` methods handling specific event types. State lives in instance fields.

For advanced cases that need full control over the projection lifecycle, implement the raw handle method:

```rust title="illustrative - raw handle method"
...
```protobuf file=proto/angzarr/cloudevents.proto region=cloud_event
```
...
```json title="illustrative - CloudEvent JSON output"
{
  "specversion": "1.0",
...
```python title="illustrative - extension attribute lowercasing"
...
```http title="illustrative - HTTP batch request"
...
```python title="illustrative - filtering sensitive data"
...
```python title="illustrative - fan-out transformation"
...
```python title="illustrative - skipping events"
def transform_events(event_book):
    cloud_events = []
...
```rust title="illustrative - framework integration"
...
- **[Projectors](/components/projector)** — Projector patterns
