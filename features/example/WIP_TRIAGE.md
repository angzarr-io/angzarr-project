...
- Batch 16: `[x]` Cluster acceptance EA-0011/0012/0013 land end-to-end against the deployed cluster (proof in `examples-python/main` — chip race emits ColorUpCompleted with conservation deltas; saga-tournament-table fans RebalanceTables out to per-table LeaveTable + SeatPlayer; eliminate_player emits HandForHandEnded alongside PlayerEliminated when the tournament is in H4H mode). Per-table `hand_for_hand_waiting` status, the saga-emitted `HandForHandRoundComplete`, and the "cannot start a new hand" guard remain pending the saga-hand-for-hand-coordinator + table-side H4H state work.
