...
- Batch 16: `[x]` Cluster acceptance EA-0011/0012/0013 land end-to-end against the deployed cluster with real assertions across all three. EA-0011: chip race emits ColorUpCompleted with conservation deltas (apply_player_enrolled seeds chip inventories from starting_stack so the race has real chips to convert). EA-0012: saga-tournament-table fans PlayerMovedBetweenTables out to per-table LeaveTable + SeatPlayer (CASCADE mode waits for both to commit). EA-0013: per-table `hand_for_hand_status` field + StartHand guard rejecting at COMPLETE; EnterTableHandForHand / MarkTableHandForHandHandComplete / EndTableHandForHand commands drive the WAITING → COMPLETE → "" lifecycle; RecordHandForHandRoundComplete on tournament emits HandForHandRoundComplete with auto-incremented round_number; eliminate_player emits HandForHandEnded alongside PlayerEliminated when the tournament is in H4H mode.
