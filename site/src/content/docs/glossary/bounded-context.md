...
Angzarr does **not** model bounded contexts. Instead, Angzarr models [domains](/glossary/domain)—aggregate namespaces and deployment units.
...
```mermaid
flowchart LR
    Team --"1:many<br/>(discouraged)"--> BC[Bounded Context]
    BC <--"1:many"--> Domains
    Domains <--"1:1"--> Aggregates
```
...
- [Domain glossary entry](/glossary/domain) for Angzarr's concept
