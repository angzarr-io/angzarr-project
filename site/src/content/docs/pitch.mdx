...
description: Detailed rationale and architecture overview for Angzarr
---
# Technical Pitch
...
```mermaid
flowchart TB
    subgraph Pod1["Pod: Aggregate"]
        subgraph YourCode1["Your Aggregate"]
            BL["BusinessLogic<br/>(Pure domain logic)"]
        end
        subgraph Sidecar1["Angzarr Sidecar (~8MB)"]
    ...
    subgraph Pod2["Pod: Saga"]
        subgraph YourCode2["Your Saga"]
            SAGA["Saga<br/>(Python)"]
        end
        subgraph Sidecar2["Angzarr Sidecar (~8MB)"]
            SC2["..."]
        end
        SAGA <-->|gRPC| SC2
    end
...
flowchart TD
    Client([Client])
    Client --> BC["BusinessCoordinator.Handle(CommandBook)"]
    BC --> BL["BusinessLogic.Handle()"]
    BL --> Events["events"]
    Events --> Persist["Persist events"]
    Persist --> SagaC["SagaCoordinator.HandleSync(events)"]
    SagaC --> Saga["Saga.HandleSync()"]
    Saga --> SagaResp["SagaResponse(commands)"]
    SagaResp --> Recursive["Recursive: each command<br/>goes through BusinessCoordinator"]
    Recursive -.-> BC
    Persist --> ProjC["ProjectorCoordinator.HandleSync<br/>(all events from cascade)"]
    ProjC --> Proj["Projector.HandleSync()"]
    Proj --> Projection["Projection"]
    Projection --> Response["CommandResponse<br/>{ events, projections[] }"]
    Response --> Client
```
...
flowchart TD
    Client([Client])
    Client --> BC["BusinessCoordinator.Handle(CommandBook)<br/>[sync_mode = SIMPLE]"]
    BC --> BL["BusinessLogic.Handle()"]
    BL --> Events["events"]
    Events --> Persist["Persist events"]
    Persist --> SagaAsync["SagaCoordinator.Handle(events)<br/>← Async, returns immediately"]
    Persist --> ProjSync["ProjectorCoordinator.HandleSync(events)<br/>← Sync, waits"]
    ProjSync --> Proj["Projector.HandleSync()"]
    Proj --> Projection["Projection"]
    Projection --> Response["CommandResponse<br/>{ events, projections[] }"]
    Response --> Client
```
...
Every command, saga, and projector execution is traced and metered at the coordinator level. The granularity is the `Handle` boundary—the exact point where the framework calls into your business logic:
...
```mermaid
flowchart LR
    subgraph Workflow["correlation_id = abc-123"]
        A["CreateOrder"] --> B["aggregate.execute"]
        B --> C["saga.orchestrate"]
        C --> D["orchestration.execute"]
        D --> E["aggregate.execute<br/>(downstream)"]
        B --> F["projector.handle"]
    end
```
